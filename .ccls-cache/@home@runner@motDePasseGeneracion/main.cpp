...
int main() {
  ...
  while (!isLogged) {
    ...
    case 1:
      ...
      if (!isLogged) {
        cout << "Invalid datum. Please try again." << endl;
      }
    ...
    case 2: {
      ...
      auto it =
          find_if(usersClass.begin(), usersClass.end(),
                  [&newId](const User &user) { return user.getId() == newId; });

      if (it != usersClass.end()) {
        cout << "ID already exists. Please choose a different ID." << endl;
        break;
      }
  ...
  do {
    ...
    case 1: {
      ...
      if (option == 1 || option == 0) {
        newPassword = gp.generate(length, lowercase, uppercase, digits,
                                  specialCharacters);
